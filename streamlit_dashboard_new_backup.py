...
# Hide Streamlit elements and apply custom CSS
st.markdown("""
<style>
...
        
        ...
        async function checkBackendHealth() {
            try {
                ...
                console.warn('⚠️ Backend not available, using AI-only mode');
            }
            return false;
        }
        
        // Analyze ECG data using backend
        async function analyzeECGWithBackend(ecgData, fs = 700, windowSec = 90, stepSec = 15) {
            try {
                const response = await fetch(`${BACKEND_API}/readiness`, {
                    method: 'POST',
        ...
        function generateSyntheticECG(durationSec = 120, fs = 700) {
            const samples = durationSec * fs;
            const ecg = [];
            const heartRate = 70; // BPM
            const samplesPerBeat = (60 / heartRate) * fs;
            ...
            for (let i = 0; i < samples; i++) {
                ...
                if (beatPhase > 0.1 && beatPhase < 0.2) {
                    // P wave
                    value = 0.2 * Math.sin((beatPhase - 0.1) * Math.PI / 0.1);
                } else if (beatPhase > 0.3 && beatPhase < 0.4) {
                    // QRS complex
                    const qrsPhase = (beatPhase - 0.3) / 0.1;
                    if (qrsPhase < 0.3) value = -0.3 * Math.sin(qrsPhase * Math.PI / 0.3);
                    else if (qrsPhase < 0.5) value = 1.5 * Math.sin((qrsPhase - 0.3) * Math.PI / 0.2);
                    else value = -0.5 * Math.sin((qrsPhase - 0.5) * Math.PI / 0.5);
                } else if (beatPhase > 0.5 && beatPhase < 0.7) {
                    // T wave
                    value = 0.3 * Math.sin((beatPhase - 0.5) * Math.PI / 0.2);
                }
...
        // --- AI FEATURES ---
        ...
        async function analyzeLiveVitals() {
            ...
            output.innerHTML = '<div class="h-12 w-full loading-shimmer rounded"></div><p class="text-xs text-center text-gray-500 mt-2">Analyzing ECG data with ML model...</p>';

            // Generate synthetic ECG and analyze with backend
            const ecgData = generateSyntheticECG(120, 700);
            const backendResult = await analyzeECGWithBackend(ecgData);
            ...
            let backendInfo = '';
            if (backendResult) {
                const avgReadiness = backendResult.summary.avg_readiness.toFixed(1);
                const state = backendResult.summary.overall_state;
                backendInfo = `\n\n**ML Model Analysis:**\n- Readiness Score: ${avgReadiness}/100\n- State: ${state.toUpperCase()}\n- Windows Analyzed: ${backendResult.summary.n_windows}\n- Score Range: ${backendResult.summary.min_readiness.toFixed(1)} - ${backendResult.summary.max_readiness.toFixed(1)}`;
            }
            ...
            - Signal Quality: ${signal}${backendInfo ? '\n            - ML Readiness Score: ' + backendResult.summary.avg_readiness.toFixed(1) + '/100' : ''}
            ...
            output.innerHTML = marked.parse(text + backendInfo);
...
        }
        ...
        window.addEventListener('load', async () => {
            ...
            if (!backendAvailable) {
                console.warn('Running in AI-only mode. Start backend with: uvicorn api:app --reload');
            }
        });
    </script>
</body>
...
components.html(dashboard_html, height=3000, scrolling=False)
