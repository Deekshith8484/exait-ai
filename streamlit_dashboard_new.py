...
def analyze_ecg_with_backend(ecg_data, fs=700, window_sec=90, step_sec=15):
    """Send ECG data to backend for readiness analysis"""
    try:
        payload = {
            "ecg": ecg_data.tolist() if isinstance(ecg_data, np.ndarray) else ecg_data,
            "fs": fs,
            "window_sec": window_sec,
            "step_sec": step_sec
        }
        response = requests.post(f"{API_BASE_URL}/readiness", json=payload, timeout=30)
...
def generate_synthetic_ecg(duration_sec=90, hrv_level='normal', noise_level=0.02):
    """Generate synthetic ECG data using ECG generator"""
    try:
        generator = ECGGenerator(fs=700, base_hr=70)
        ecg_data = generator.generate_segment(
        ...
        st.error(f"ECG generation error: {str(e)}")
        return None
...
if page == "🏠 Home":
    ...
    st.markdown(f"""
    <div style="background: {status_color}; color: white; padding: 0.5rem 1rem; border-radius: 0.5rem; text-align: center; margin-bottom: 1rem; font-weight: 600;">
        {status_text} | API: {API_BASE_URL}
    </div>
    """, unsafe_allow_html=True)
...
# ============================================================================
    ...
    # Synthetic ECG Generation and Analysis
    st.markdown("### 🔬 Real-Time ECG Analysis with ML Backend")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        hrv_level = st.selectbox("HRV Level", ["high", "normal", "low"], index=1)
    with col2:
        duration = st.slider("Duration (sec)", 30, 180, 90)
    with col3:
        noise_level = st.slider("Noise Level", 0.0, 0.1, 0.02, 0.01)
    
    if st.button("🚀 Generate & Analyze ECG", type="primary", disabled=not backend_online):
        if not backend_online:
            st.error("❌ Backend is offline. Start the API server first: `python api.py`")
        else:
            with st.spinner("Generating synthetic ECG and analyzing..."):
                # Generate ECG
                ecg_data = generate_synthetic_ecg(duration_sec=duration, hrv_level=hrv_level, noise_level=noise_level)
                
                if ecg_data is not None:
                    # Analyze with backend
                    result = analyze_ecg_with_backend(ecg_data, fs=700, window_sec=min(90, duration), step_sec=15)
                    ...
                    if result:
                        ...
                        with col4:
                            state_emoji = "🟢" if result['overall_state'] == 'ready' else "🟡" if result['overall_state'] == 'neutral' else "🔴"
                            st.metric("Overall State", f"{state_emoji} {result['overall_state'].upper()}")
                        
                        # Plot ECG Signal
                        st.markdown("#### ECG Signal")
                        time_axis = np.arange(len(ecg_data)) / 700.0  # 700 Hz sampling rate
                        fig = go.Figure()
                        fig.add_trace(go.Scatter(x=time_axis, y=ecg_data, mode='lines', name='ECG', line=dict(color='#10B981', width=1)))
                        fig.update_layout(
                        ...
                        st.markdown("#### Readiness Progression")
                        windows = result['window_results']
                        times = [w['center_time_sec'] for w in windows]
                        readiness = [w['readiness'] for w in windows]
                        
                        fig2 = go.Figure()
                        fig2.add_trace(go.Scatter(x=times, y=readiness, mode='lines+markers', name='Readiness', 
                                                 line=dict(color='#6366F1', width=2), marker=dict(size=6)))
                        fig2.add_hline(y=70, line_dash="dash", line_color="green", annotation_text="Ready Threshold")
                        fig2.add_hline(y=40, line_dash="dash", line_color="orange", annotation_text="Caution Threshold")
                        fig2.update_layout(
